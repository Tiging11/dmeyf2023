...
PARAM$experimento <- "KA_baseline_param_adic_seed_50" 
...
PARAM$input$dataset <- "./datasets/baseline_exp_colab.csv.gz"
...
PARAM$finalmodel$learning_rate <- 0.2847162
PARAM$finalmodel$feature_fraction <- 0.9141471
PARAM$finalmodel$min_data_in_leaf <- 20101
PARAM$finalmodel$num_leaves <- 120
PARAM$finalmodel$max_bin <- 31

# Parámetros adicionales:
PARAM$finalmodel$bagging_fraction <- 0.972449
PARAM$finalmodel$pos_bagging_fraction <- 0.1966233
PARAM$finalmodel$neg_bagging_fraction <- 0.8471233
PARAM$finalmodel$bagging_freq <- 20
PARAM$finalmodel$feature_fraction_bynode <- 0.7721488
PARAM$finalmodel$max_depth <- 32
...
for (semilla in semillas) {
  ...
  modelo <- lgb.train(
    ...
    param = list(
      ...
      pos_bagging_fraction = PARAM$finalmodel$pos_bagging_fraction,
